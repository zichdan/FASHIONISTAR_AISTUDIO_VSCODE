# apps/common/providers/SMTP/zoho.py
...
class ZohoEmailProvider:
    """
    Email provider using Zoho Mail via django-anymail.
    Sends both plain text and HTML emails with full template support.
    """

    def __init__(self):
        """
        Initialize Zoho provider with configuration from settings.
        """
    ...
    def send(self, subject: str, recipients: list, context: dict = None, template_name: str = None, message: str = None, attachments: list = None, fail_silently: bool = False) -> None:
        """
        Send email synchronously via Zoho Mail.
        ...
        try:
            ...
            logger.info(f"Email sent via Zoho to {recipients}")
        except Exception as e:
            logger.error(f"Error sending email via Zoho: {str(e)}")
    ...
    async def asend(self, subject: str, recipients: list, context: dict = None, template_name: str = None, message: str = None, attachments: list = None, fail_silently: bool = False) -> None:
        """
        Send email asynchronously via Zoho Mail.
        ...
        try:
            ...
            logger.info(f"Email sent (async) via Zoho to {recipients}")
        except Exception as e:
            logger.error(f"Error sending email (async) via Zoho: {str(e)}")
            if not fail_silently:
                raise
