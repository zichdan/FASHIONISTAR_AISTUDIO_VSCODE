...
Architecture:
- Uses DRF's exception_handler as the base to intercept DRF-specific exceptions.
- Extends with custom handlers for Auth, Validation, and Permission errors.
- Logs all 4xx (Client Errors) and 5xx (Server Errors) with IP, endpoint, user context.
...
Compliance:
    ...
    ✅ Comprehensive Logging (Application Context)
    ✅ Structured Error Responses (JSON)
    ✅ Audit Trail for Security Events
    ✅ Supports Async/Sync Contexts
"""
...
def custom_exception_handler(exc, context):
    ...
    Features:
    1. DRF Exception Handling: Calls DRF's base handler first.
    2. Custom Exception Mapping: Maps Django/Python exceptions to HTTP status codes.
    3. Audit Logging: Logs all errors with IP, endpoint, user, timestamp.
    4. Debug Mode: Includes full traceback only in DEBUG mode.
    5. Rate Limit Handling: Special formatting for Throttled exceptions.

    Args:
        exc (Exception): The exception that was raised.
        context (dict): Contains 'view', 'request', 'args', 'kwargs'.

    Returns:
        Response: Standardized error response or None.
    """
    try:
        # ====================================================================
...
def _get_client_ip(request):
    ...
    Checks in order:
    1. X-Forwarded-For header (proxy chain)
    2. X-Real-IP header (common proxy)
...
def _log_error(message, level=logging.ERROR, status_code=500, exception=None):
    ...
    Args:
        ...
        level (int): Logging level (logging.ERROR, logging.WARNING, etc.).
